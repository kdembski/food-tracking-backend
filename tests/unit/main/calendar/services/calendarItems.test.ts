...
jest.mock("@/main/calendar/services/calendarItemMembers", () => ({
  CalendarItemMembersService: jest
    .fn()
    .mockImplementation(() => ({ addCalendarItemToMembers })),
}));

const updateOrderedFoodLastDate = jest.fn();
jest.mock("@/main/calendar/services/calendarItemOrderedFood", () => ({
  CalendarItemOrderedFoodService: jest
    .fn()
...
jest.mock("@/main/calendar/services/calendarItemRecipes", () => ({
  CalendarItemRecipesService: jest
    .fn()
...
describe("Calendar Items Service", () => {
  ...
  it("Should calendar items grouped by days", async () => {
    const calendarDays = await service.getDays(
      new Date(2000, 1, 0),
      new Date(2000, 1, 3),
      [1]
    );
  ...
  it("Should update last date for linked item", async () => {
    ...
    await service.create(new CalendarItem(dto));

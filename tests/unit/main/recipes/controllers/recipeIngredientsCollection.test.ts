...
describe("Recipe Ingredients Collection Controller", () => {
  let controller: RecipeIngredientsCollectionController;

  beforeEach(() => {
    controller = new RecipeIngredientsCollectionController();
  });

  it("Should call repository selectByRecipeId on getByRecipeId call", async () => {
    expect(await controller.getByRecipeId(1)).toEqual(
  ...
  it("Should calculate kcal and call updateKcal from RecipeController on callback call", async () => {
    expect(await controller.callback(1));
